...
const paramCall = paramDecorator => t.callExpression(t.identifier('akessrfljlrgqgd_param'), [t.numericLiteral(paramDecorator.index), paramDecorator.decorator.expression]);
...
function applyParamDecoratorExpression(methodDecorators, paramDecorators, classDeclaration, method) {
  ...
  return t.callExpression(t.identifier('akessrfljlrgqgd_decorate'), [paramCalls, t.memberExpression(classDeclaration.id, t.identifier('prototype')), t.stringLiteral(method.key.name), t.identifier('null')]);
}